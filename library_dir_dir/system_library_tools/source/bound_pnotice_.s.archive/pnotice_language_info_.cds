...
/* HISTORY COMMENTS:
  1) change(1981-05-14,Stansbury), approve(1981-05-14,Unknown),
     audit(1981-05-14,Unknown), install(1986-08-21,MR12.0-1138):
     Created.
  2) change(1982-02-02,Stansbury), approve(1982-02-02,Unknown),
     audit(1982-02-02,Unknown), install(1986-08-21,MR12.0-1138):
     Modified - Added the 'pmac' suffix for the PL/1 macro expander.
  3) change(1982-06-04,Stansbury), approve(1982-06-04,Unknown),
     audit(1982-06-04,Unknown), install(1986-08-21,MR12.0-1138):
     Modified - Added the 'ld' suffix for the library descriptor language.
  4) change(1982-06-17,Stansbury), approve(1982-06-17,Unknown),
     audit(1982-06-17,Unknown), install(1986-08-21,MR12.0-1138):
     Modified - Added the following suffixes: pascal, cmdb, header, mexp,
     runoff, linus
  5) change(1982-09-28,Stansbury), approve(1982-09-28,Unknown),
     audit(1982-09-28,Unknown), install(1986-08-21,MR12.0-1138):
     Modified - Added the ted and qedx suffixes.
  6) change(1985-09-03,LJAdams), approve(1985-09-27,MCR7150),
     audit(1986-02-07,Wallman), install(1986-02-13,MR12.0-1017):
     ...
     by compose.
  7) change(1985-11-13,LJAdams), approve(1985-11-13,MCR7150),
     audit(1986-02-07,Wallman), install(1986-02-13,MR12.0-1017):
     Changed pascal begin/end delimiters to (* and *) respectively.  Added xdw
     language type suffix.  Changed pmac suffix to type 4.
  8) change(1986-01-29,LJAdams), approve(1986-01-29,MCR7150),
     audit(1986-02-07,Wallman), install(1986-02-13,MR12.0-1017):
     Added support for "C" language and micro_assembler.
  9) change(1986-07-28,LJAdams), approve(1986-08-01,MCR7509),
     audit(1986-08-05,Blair), install(1986-08-21,MR12.0-1138):
     Added support for lap language.
 10) change(1986-09-08,LJAdams), approve(1986-09-08,MCR7526),
     audit(1986-11-05,GDixon), install(1986-11-12,MR12.0-1213):
     Added .cmf, .ttf, .rtmf, .ssl, .teco, and iodt suffixes.
 11) change(1986-09-23,LJAdams), approve(1986-09-23,MCR7526),
     audit(1986-11-05,GDixon), install(1986-11-12,MR12.0-1213):
     Added support for bind_fnp suffix.
 12) change(1987-03-16,LJAdams), approve(1987-04-22,MCR7653),
     audit(1987-04-02,Gilcrease), install(1987-04-26,MR12.1-1026):
     Added support for C language header files (.H) files. (phx20795)
 13) change(2019-08-13,GDixon), approve(2019-08-13,MCR10065),
     audit(2019-09-20,Swenson), install(2019-09-20,MR12.6g-0030):
     Add an entry describing comment start and end strings for info segments.
     This supports adding history comments in a separate :Internal: chunk of an
     info segment.
...
dcl 1 lang_info aligned,
      2 languages,
        3 N fixed bin,
        3 lang_array (42) like pnotice_language_info.lang_array;
	...
	lang_info.comment_end(41) = "*/";
          ...
          lang_info.lang_type(42) = 4;
	lang_info.lang_name(42) = "info";		/* INFO */
	lang_info.comment_start(42) = "/****^ ";
	lang_info.comment_end(42) = "*/";
