...
/* HISTORY COMMENTS:
  1) change(1974-07-04,GDixon), approve(), audit(), install():
     Version 1.0--
      Created initial version of the program.
  2) change(1975-02-28,GDixon), approve(), audit(), install():
     Version 2.0--
      Remove Global keywords; make root keyword more flexible;
      rename it to Root
  3) change(1975-12-01,GDixon), approve(), audit(), install():
     Version 2.1--
  ...
  4) change(1976-05-25,GDixon), approve(), audit(), install():
     Version 3.0--
  ...
  7) change(1986-01-14,GDixon), approve(1986-02-06,MCR7336),
     audit(1986-02-11,Dickson), install(1986-02-11,MR12.0-1015):
     Version 3.3--
      corrects a bug which prevents library root names from having
      more than two components.
  8) change(2019-08-13,GDixon), approve(2020-01-04,MCR10069.1),
     install(2020-01-20,MR12.6g-0034):
     Version 3.4--  Modernize library_descriptor_compiler user interface:
      - Accept input either as a standalone segment or archive component.
      - Accept and handle more than one control argument.
      - Accept -trace on|off control argument to enable/disable
        tracing of reductions as they process the library descriptor file.
      - Upon completion of a successful parse of the input, automatically
        invoke alm to compile the output into a library descriptor object.
      - Use a new ldc_severity_ external variable, which is set to max severity
        encountered by errors in either ldc or alm.  This variable enables
        the severity command/AF to report status of the last ldc translation.
...
ldc:	procedure;
	...
	Icommand			fixed bin,	/* index of the command in command default values	*/
						/*    struc.				*/
          Iarg			fixed bin,	/* index of current argument in arg list.	*/
	...
	alm_args			char(300) varying,	/* Args to be passed to alm compiler.		*/
	...
	comp_in			char(32),		/* component part of path name of input segment.	*/
...
%page;
	...
	expand_pathname_$component_add_suffix 
				entry (char(*), char(*), char(*), char(*), char(*), fixed bin(35)),
	...
	tssi_$get_segment		entry (char(*), char(*), ptr, ptr, fixed bin(35));
     ...
     dcl						/*	static variables			*/
         (F init("0"b), T init("1"b))   bit(1) aligned int static options(constant),
...
"),
	...
	alm_severity_		fixed bin(35) external static init(0),
...
"),							/* SP HT NL NP				*/
	...
	ldc_severity_		fixed bin(35) external static init(0),
...
/*  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  **  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  */
	...
	call expand_pathname_$component_add_suffix (arg, "ld", dir_in, ent_in, comp_in, code);
	if code ^= 0 then go to bad_path;		/* expand path name to absolute form.		*/

	dir_out = get_wdir_();			/* put output segment in working directory.	*/
	if comp_in ^= "" then
	     call suffixed_name_$new_suffix (comp_in, "ld", "alm", ent_out, code);
	else call suffixed_name_$new_suffix (ent_in,  "ld", "alm", ent_out, code);
	if code ^= 0 then go to bad_output_name;	/* ensure name of output segment is suffixed OK.	*/

	alm_args = "";
	do Iarg = 2 to Nargs;			/* process any control argument.		*/
		...
		Iarg = Iarg + 1;
		call cu_$arg_ptr (Iarg, Parg, Larg, code);
		if      arg = "on"  then TRACING = T;
		else if arg = "off" then TRACING = F;
		else go to badtrace;
		end;
	     else if arg = "-list" | arg = "-ls" then
		alm_args = alm_args || " " || arg;
	     else
		go to badopt;
	     end;
...
cleaner:	procedure;				/* This is a cleanup procedure.		*/
	...
	if Pout ^= null then			/* clean up out segment.			*/
	     call tssi_$clean_up_segment (Pacl_out);
          ...
          call initiate_file_$component (dir_in, ent_in, comp_in, R_ACCESS, Pin, bc_in, code);
...
RETURN:	if MERROR_SEVERITY > 2 then do;		/* Fatal error?  No output created.		*/
	...
	ldc_severity_ = MERROR_SEVERITY;
	if code ^= 0 then go to error;		/* report any errors to user.			*/

	alm_args = "[where alm]$alm " || pathname_ (dir_out, ent_out) || alm_args;
	call cu_$cp (addr(substr(alm_args,1)), length(alm_args), code);
	ldc_severity_ = max(MERROR_SEVERITY, alm_severity_);

	if alm_severity_ <= 1 then do;

  dcl 1 delS aligned like delete_options;
	     string (delS) = F;
	     delS.segment, delS.raw = T;
               call delete_$path (dir_out, ent_out, string(delS), "", code);
	     end;
...
badtrace:	call com_err_ (error_table_$badopt, proc, "-trace ^a", arg);
	return;
...
bad_input:
	...
	call com_err_ (code, proc, " ^a", pathname_$component (dir_in, ent_in, comp_in));
	return;
